...
def check_line_and(line):
    if len(line) > 0:
        line += ' && '
    return line


def check_line_or(line):
    if len(line) > 0:
        line += ' || '
    return line
...
class Group:
    ...
    * depends: 依赖条件, 包括menu和choice的depends以及if组关系
    ...
    def set_group_display(self, value):
        self.display = check_line_and(self.display)
        self.display = value

    def set_group_dep(self, value):
        self.depends = check_line_and(self.depends)
        self.depends = value
...
class ConfigDep:
    ...
    * restrict: 
    ...
    def set_select(self, value):
        self.rev_select = check_line_or(self.rev_select)
        self.rev_select += value

    def set_depends(self, value):
        if len(value) > 0:
            self.dep = check_line_and(self.dep)
            self.dep += value

    def set_restrict(self, value, if_expr):
        self.restrict = check_line_or(self.restrict)
        if len(if_expr) > 0:
    ...
    def set_imply(self, father, if_expr):
        ...
        if len(save) > 0:
            self.restrict = check_line_or(self.restrict) + save
...
class Node:
    ...
    def __init__(self, name, type, path) -> None:
        ...
        self.detail = self.gen_detail()
        self.config_dep = self.gen_dep()
    ...
    def gen_dep(self):
        if self.type == 'config' or self.type == 'menuconfig' or self.type == 'choice':
            return ConfigDep()
